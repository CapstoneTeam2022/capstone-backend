...
import { HealthCenterDto } from 'src/health-center/dto';
...
export class ResearcherService {
  ...
  async getHealthCenterAnalytics(healthcenter: string) {
    let users =[];
    const health_centers = this.healthCenterService.getAllHealthCenterswithUsers();
  ...
  async getHealthCenterUsers(users: User[]) {
    ...
    (await (allUsers)).map((user) => {
      (users).map((singleUser) => {
        if (singleUser.id === singleUser.id) {
  ...
  async getVitals(body: (DiseaseAnalytics| MedicationAnalytics), datas: InvestigationRequest[]) {
  ...
  async getUsers(body: (DiseaseAnalytics| MedicationAnalytics), datas: Vitals[]) {
  ...
  async getMedicationAnalytics(body: MedicationAnalytics) {
    ...
    const prescription_length = (await this.prescriptionService.findAll()).length;
    ...
    datas['total_prescriptions'] = prescription_length;
    datas['medicatedPatientCount'] = medicatedPatientCount;
    datas['byDateCount']= byDateCount;
  ...
  async getPatientRecord() {
    ...
    (await patients).map((patient) => {
      ...
      if (patient.user.gender === 'male' || patient.user.gender === 'Male') {
        ...
        patient.user.gender === 'Male to Female' ||
  ...
  async getUserRecord() {
    ...
    const users = this.userService.getAllUsers();
    (await users).map((user) => {
      if (user.role.name === 'Doctor' || user.role.name === 'repellat') {
        doctor = doctor + 1;
      } else if (user.role.name === 'Nurse' || user.role.name === 'ut') {
        nurse = nurse + 1;
      } else if (
        user.role.name === 'repellat' ||
        user.role.name === 'Hospital admin' ||
        user.role.name === 'hospital Admin' ||
        user.role.name === 'hospital admin'
      ) {
      ...
      if (user.gender === 'male' || user.gender === 'Male to Female') {
        male = male + 1;
      } else if (user.gender === 'female' || user.gender === 'Female') {
        female = female + 1;
      }
